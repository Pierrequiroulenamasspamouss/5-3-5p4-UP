...
{
	public class LzwInputStream : global::System.IO.Stream
	{
		...
		public LzwInputStream(global::System.IO.Stream baseInputStream)
		{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			if (num10 > 0)
			{
				...
				global::System.Array.Copy(array3, num7, buffer, offset, num11);
				offset += num11;
				count -= num11;
				num7 += num11;
			}
				...
				while (true)
				{
					if (num9 < num12)
					{
						...
						if (num6 == -1)
						{
							if (num15 >= 256)
							{
								throw new global::ICSharpCode.SharpZipLib.LZW.LzwException("corrupt input: " + num15 + " > 255");
							}
							b = (byte)(num6 = num15);
							buffer[offset++] = b;
							count--;
						...
						{
							global::System.Array.Copy(zeros, 0, array, 0, zeros.Length);
							num8 = 256;
							int num16 = num2 << 3;
						...
						if (num15 >= num8)
						{
							if (num15 > num8)
							{
								throw new global::ICSharpCode.SharpZipLib.LZW.LzwException("corrupt input: code=" + num15 + ", freeEnt=" + num8);
							}
							array3[--num7] = b;
							num15 = num6;
						}
						...
						global::System.Array.Copy(array3, num7, buffer, offset, num18);
						offset += num18;
						count -= num18;
						num7 += num18;
						if (num8 < num4)
		...
		private int ResetBuf(int bitPosition)
		{
			...
			global::System.Array.Copy(data, num, data, 0, end - num);
			end -= num;
			return 0;
		}
		...
		private void ParseHeader()
		{
			...
			if (num < 0)
			{
				throw new global::ICSharpCode.SharpZipLib.LZW.LzwException("Failed to read LZW header");
			}
			if (array[0] != 31 || array[1] != 157)
			{
				throw new global::ICSharpCode.SharpZipLib.LZW.LzwException(string.Format("Wrong LZW header. Magic bytes don't match. 0x{0:x2} 0x{1:x2}", array[0], array[1]));
			}
			blockMode = (array[2] & 0x80) > 0;
			maxBits = array[2] & 0x1F;
			if (maxBits > 16)
			{
				throw new global::ICSharpCode.SharpZipLib.LZW.LzwException("Stream compressed with " + maxBits + " bits, but decompression can only handle " + 16 + " bits.");
			}
			if ((array[2] & 0x60) > 0)
			{
				throw new global::ICSharpCode.SharpZipLib.LZW.LzwException("Unsupported bits set in the header.");
			}
