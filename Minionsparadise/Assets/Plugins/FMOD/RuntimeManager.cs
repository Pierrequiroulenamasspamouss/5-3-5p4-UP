        ...
        void Initialiase(bool forceNoNetwork)
        {
            ...
            fmodPlatform = RuntimeUtils.GetCurrentPlatform();            
                ...
                UnityEngine.Debug.LogWarning(
                    string.Format(
                        "FMOD Studio: Cannot open FMOD debug log file '{0}', logs will be missing for this session.",
                        System.IO.Path.Combine(Application.dataPath, RuntimeUtils.LogFileName)
                    )
                    );
            }
                else
